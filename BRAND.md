# Revealing Leads to Healing Wellness Services, LLC — Brand Specification

**This is the single source of truth for the brand. Every surface — website,
EHR, documents, emails — must use these values. Nothing may substitute or
override them (no teal, no navy, no reinterpretation).**

Owner: Kenseener "Kay" Carpenter, LCSW, CCTP, CGP, CASAC-M, SIFI
Practice: Revealing Leads to Healing Wellness Services, LLC

---

## Colors (the brand — as defined by the owner)

| Name           | Use                                   | Hex (confirmed from logo) |
|----------------|---------------------------------------|---------------------------|
| Champagne gold | Primary accent, logo script, heading highlights | `#EBC94E` |
| Charcoal grey  | Primary text, secondary elements      | `#3A3A3A` |
| Bright white   | Backgrounds                           | `#FFFFFF` |
| Black          | Outlines, high-contrast detail        | `#111111` |

CONFIRMED by the owner as the exact brand colors, sampled from the official
logo. These are the definitive values. Nothing may substitute them.

---

## Fonts

| Role      | Font        | Notes |
|-----------|-------------|-------|
| Body text | Montserrat  | Free Google Font. Confirmed from the owner's original Webador site source. |
| Headings  | Bevan       | Free Google Font. Confirmed from the owner's original Webador site source (`family=Bevan|Montserrat`). NOTE: previously referred to as "Belvin" — the real font is **Bevan**. Both fonts are free and embeddable, no font file needed. |

---

## Logo

- Round logo: gold "R H" script, "REVEALING LEADS TO HEALING" in charcoal,
  centered line-art figure, "COUNSELING & WELLNESS SERVICES, LLC" beneath,
  thin black circle border, transparent/white background.
- Use in site header and EHR header for one consistent brand.

---

## Owner name & credentials (use exactly)

- Name in full: **Kenseener "Kay" Carpenter** — "Kay" always in quotes (Kenseener
  is the professional/legal name; Kay is the known name).
- Credentials: **LCSW, CCTP, CGP, CASAC-M, SIFI**. Fully licensed and certified
  across several clinical areas and modalities, with international credentials,
  master-level supervisory certification (the "M" in CASAC-M = supervision), and
  SIFI certification. Do not drop or abbreviate these.

## Content that must remain (owner's June/July 2026 updates)

- The June/July 2026 site changes STAY — made when the owner became fully
  licensed. This includes the profile picture and updated bio/credentials.
  Do NOT revert to the original 2024 Webador creation.
- **Music stays.** The uplifting songs are intentional, positive, and personally
  chosen by the owner. They are a deliberate part of the brand and patient
  experience. Keep the music feature.

## Rules

1. These colors and fonts apply to BOTH the public website and the EHR, for one
   aligned brand — the owner's original all-in-one vision (cohesive, professional,
   easy access from site to EHR).
2. No color or font may be substituted for convenience or "improvement."
3. The owner's name, credentials, June/July updates, profile picture, and music
   are not to be changed or removed.
4. Any change to the brand is made HERE first, then propagated. This file wins.
