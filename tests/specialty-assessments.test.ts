...
const base = { administrationDate: '2026-09-01', examiner: 'Test Provider', referralStatus: 'No referral indicated at this time', interpretation: 'Reviewed', followUp: 'Follow up scheduled' };
...
test('referral decisions require destination and reason, and reject invalid numeric scores', () => {
  const data = { ...base, version: 'test', respondent: 'test', results: 'test', source: 'test', referralStatus: 'Referral recommended' };
  assert.ok(validateSpecialtyAssessment('psc', data));
  assert.equal(validateSpecialtyAssessment('psc', { ...data, referralDestination: 'Specialist', referralReason: 'Further evaluation', totalScore: '0' }), '');
  assert.ok(validateSpecialtyAssessment('psc', { ...data, referralDestination: 'Specialist', referralReason: 'Further evaluation', totalScore: 'invalid' }));
});
